...
df_rq2 <- df %>%
  select(date, pm25_london_mean, temp_mean_c, wind_mean_ms, prcp_sum_mm) %>%
  filter(
...
rho_mat <- cor(
  df_rq2 %>% select(pm25_london_mean, temp_mean_c, wind_mean_ms, prcp_sum_mm),
  method = "spearman",
  use = "complete.obs"
)
rho_mat
...
rc <- rcorr(
  as.matrix(df_rq2 %>% select(pm25_london_mean, temp_mean_c, wind_mean_ms, prcp_sum_mm)),
  type = "spearman"
)
rc  # shows r, n, P

cor_table <- tibble(
  variable = c("temp_mean_c", "wind_mean_ms", "prcp_sum_mm"),
  spearman_rho = c(
    rc$r["pm25_london_mean","temp_mean_c"],
    rc$r["pm25_london_mean","wind_mean_ms"],
    rc$r["pm25_london_mean","prcp_sum_mm"]
  ),
  p_value = c(
    rc$P["pm25_london_mean","temp_mean_c"],
    rc$P["pm25_london_mean","wind_mean_ms"],
    rc$P["pm25_london_mean","prcp_sum_mm"]
  ),
  n = c(
    rc$n["pm25_london_mean","temp_mean_c"],
    rc$n["pm25_london_mean","wind_mean_ms"],
...
# PM2.5 vs windspeed
p_wind <- ggplot(df_rq2, aes(x = wind_mean_ms, y = pm25_london_mean)) +
...
df_rq2b <- df_rq2 %>%
  mutate(
    wind_group = ntile(wind_mean_ms, 3),
    wind_group = factor(
      wind_group,
      levels = c(1, 2, 3),
