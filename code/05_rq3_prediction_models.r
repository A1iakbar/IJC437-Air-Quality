...
lm_fit <- lm_spec %>%
  fit(pm25_london_mean ~ pm25_lag_1 + pm25_roll7 + pm25_roll30 +
        temp_mean_c + wind_mean_ms + prcp_sum_mm +
...
if (requireNamespace("lightgbm", quietly = TRUE)) {
  ...
  print("LightGBM detected. Fitting LGBM model...")
  ...
  # Predict on TEST data (holdout)
  lgbm_test_pred <- predict(lgbm_fit, test_data) %>%
    bind_cols(
      test_data %>%
        select(date, pm25_london_mean)
    ) %>%
    rename(
      actual = pm25_london_mean,
      pred   = .pred
    )

  # Evaluation metrics
  lgbm_metrics <- lgbm_test_pred %>%
    yardstick::metrics(truth = actual, estimate = pred) %>%
    filter(.metric %in% c("rmse", "mae", "rsq")) %>%
    mutate(model = "LightGBM")
}else {
  print("LightGBM NOT installed. Please install LGBM first")
...
metrics_csv <- bind_rows(lm_metrics, rf_metrics, lgbm_metrics, xgb_metrics, naive_metrics) %>%
