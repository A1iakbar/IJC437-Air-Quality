...
weather_daily <- tibble(
  ...
  wind_mean_ms = weather_json$daily$wind_speed_10m_mean,
...
qa_summary <- tibble(
  ...
  value = c(
    ...
    sum(!is.na(merged$wind_mean_ms)),
    sum(!is.na(merged$prcp_sum_mm)),
    sum(complete.cases(merged %>% select(pm25_london_mean, temp_mean_c, wind_mean_ms, prcp_sum_mm))),
